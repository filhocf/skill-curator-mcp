...
class TestSkillMatch:
    def test_returns_top_k_ordered_by_score_desc(
        self, db_with_skills: Database
    ) -> None:
        results = skill_match(
            "build an API", db=db_with_skills, encoder=MockEncoder(), top_k=2
        )
    ...
    def test_excludes_archived_skills(self, db_with_skills: Database) -> None:
        results = skill_match(
            "deprecated tool", db=db_with_skills, encoder=MockEncoder(), top_k=10
        )
        names = [r["name"] for r in results]
        assert "archived-skill" not in names

    def test_profile_boost_increases_score(self, db_with_skills: Database) -> None:
        without_profile = skill_match(
            "REST API", db=db_with_skills, encoder=MockEncoder(), top_k=5
        )
        with_profile = skill_match(
            "REST API",
            db=db_with_skills,
            encoder=MockEncoder(),
            top_k=5,
        ...
        score_without = next(
            r["score"] for r in without_profile if r["name"] == "python-rest"
        )
        score_with = next(
            r["score"] for r in with_profile if r["name"] == "python-rest"
        )
...
class TestSkillFeedback:
    def test_saves_feedback_and_updates_effectiveness_ema(
        self, db_with_skills: Database
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "outcome,direction", [("success", "up"), ("failure", "down")]
    )
    def test_effectiveness_direction(
        self, db_with_skills: Database, outcome: str, direction: str
    ) -> None:
        old_eff = db_with_skills.get_skill("testing").effectiveness  # 0.6
        skill_feedback(
            "testing", outcome=outcome, task_description="task", db=db_with_skills
        )
    ...
    def test_nonexistent_skill_returns_error(self, db: Database) -> None:
        result = skill_feedback(
            "nonexistent", outcome="success", task_description="x", db=db
        )
...
class TestSkillGaps:
    def test_returns_skills_with_gap_count_gt_zero(self, db: Database) -> None:
        db.upsert_skill(
            Skill(name="gapped", path="/g.md", gap_count=3, state=LifecycleState.ACTIVE)
        )
        db.upsert_skill(
            Skill(name="ok", path="/ok.md", gap_count=0, state=LifecycleState.ACTIVE)
        )
...
class TestSkillLifecycle:
    def test_categorizes_active_stale_promote_archive(
        self, db_with_skills: Database
    ) -> None:
...
class TestSkillPromote:
    def test_changes_state_to_active(self, db_with_skills: Database) -> None:
        skill_promote("old-skill", db=db_with_skills)
...
class TestSkillArchive:
    def test_changes_state_to_archived(self, db_with_skills: Database) -> None:
        skill_archive("testing", db=db_with_skills)
...
class TestOnboardingGuide:
    ...
    def test_full_guide_has_lifecycle(self, db: Database) -> None:
        ...
        assert "lifecycle" in result, (
            f"Missing 'lifecycle' key. Keys: {list(result.keys())}"
        )
    ...
    def test_full_guide_has_thresholds(self, db: Database) -> None:
        ...
        assert "thresholds" in result, (
            f"Missing 'thresholds'. Keys: {list(result.keys())}"
        )
    ...
    def test_compact_has_lifecycle_and_protocol(self, db: Database) -> None:
        ...
        assert "lifecycle" in result, (
            f"Compact missing 'lifecycle'. Keys: {list(result.keys())}"
        )
        assert "integration_protocol" in result, (
    ...
    def test_tools_list_includes_skill_match(self, db: Database) -> None:
        ...
        result = get_onboarding_guide(verbosity="full", db=db)
        tools = result["tools"]
        assert "skill_match" in tools, (
            f"'skill_match' not in tools. Keys: {list(tools.keys())}"
        )
...
class TestSkillMatchSuggestion:
    ...
    def test_high_score_no_suggestion(self, db: Database) -> None:
        ...
        assert results[0]["score"] >= 0.7, (
            f"Expected score >= 0.7, got {results[0]['score']}"
        )
    ...
    def test_mid_score_improvement_opportunity(self, db: Database) -> None:
        ...
        skill = Skill(
            ...
            path="/s/t.md",
            description="Testing strategies partial",
            effectiveness=0.5,
            state=LifecycleState.ACTIVE,
        )
        db.upsert_skill(skill)
        db.save_embedding(
            "testing-patterns", encoder.encode("Testing strategies partial")
        )
        results = skill_match("python rest endpoint", db=db, encoder=encoder, top_k=3)
        # Find a result with mid-range score
        mid_results = [r for r in results if 0.5 <= r["score"] < 0.7]
        assert len(mid_results) >= 1, (
            f"Expected mid-score result, got scores: {[r['score'] for r in results]}"
        )
    ...
    def test_gap_detected_logs_to_gap_log(self, db: Database) -> None:
        ...
        cur = db.conn.execute(
            "SELECT task_description, best_match_name, best_match_score FROM gap_log"
        )
        rows = cur.fetchall()
        assert len(rows) >= 1
        row = rows[-1]
...
class ClusteringEncoder:
    ...
    def encode(self, text: str) -> list[float]:
        ...
        if any(
            w in lowered
            for w in ["linkedin", "social media", "publish", "post", "share"]
        ):
...
def db_with_gap_log(db: Database) -> Database:
    ...
    for desc in [
        ...
        db.add_gap_log(
            ...
            best_match_name="personal-branding",
...
class TestSkillGapsCorrelation:
    ...
    def test_correlate_false_returns_current_behavior(
        self, db_with_gap_log: Database
    ) -> None:
    ...
    def test_correlate_true_returns_detected_patterns(
        self, db_with_gap_log: Database
    ) -> None:
    ...
    def test_clusters_similar_tasks(self, db_with_gap_log: Database) -> None:
        ...
        assert len(large_clusters) >= 1, (
            f"Expected cluster with >=3 occurrences, got: {patterns}"
        )

    def test_dissimilar_tasks_not_clustered(
        self, db_with_diverse_gaps: Database
    ) -> None:
        ...
        assert len(large_clusters) == 0, (
            f"Expected no large clusters, got: {large_clusters}"
        )
    ...
    def test_actionable_flag_when_gte_3(self, db_with_gap_log: Database) -> None:
        ...
        for cluster in large_clusters:
            assert cluster["actionable"] is True, (
                f"Expected actionable=True, got {cluster}"
            )

    def test_recommended_action_create_when_low_score(
        self, db_with_low_score_cluster: Database
    ) -> None:
        """Cluster where avg best_match_score < 0.3 → recommended_action = 'create_skill'."""
        encoder = ClusteringEncoder()
        result = skill_gaps(
            correlate=True, db=db_with_low_score_cluster, encoder=encoder
        )
        patterns = result["detected_patterns"]
    ...
    def test_recommended_action_evolve_when_mid_score(
        self, db_with_mid_score_cluster: Database
    ) -> None:
        """Cluster where avg best_match_score 0.3-0.6 → recommended_action = 'evolve_skill'."""
        encoder = ClusteringEncoder()
        result = skill_gaps(
            correlate=True, db=db_with_mid_score_cluster, encoder=encoder
        )
        patterns = result["detected_patterns"]
        assert len(patterns) >= 1, "Expected at least 1 pattern"
        actionable = [p for p in patterns if p.get("actionable")]
    ...
    def test_recommended_action_scout_when_high_score(
        self, db_with_high_score_cluster: Database
    ) -> None:
        """Cluster where avg best_match_score >= 0.6 → recommended_action = 'scout_external'."""
        encoder = ClusteringEncoder()
        result = skill_gaps(
            correlate=True, db=db_with_high_score_cluster, encoder=encoder
        )
        patterns = result["detected_patterns"]
        assert len(patterns) >= 1, "Expected at least 1 pattern"
        actionable = [p for p in patterns if p.get("actionable")]
